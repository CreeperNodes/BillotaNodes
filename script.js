...
// Panels dropdown: Game, VPS and Web panels
const panelDropdown=$('.panel-dropdown');
const panelSelect=$('.panel-select');
panelSelect?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const open=panelDropdown.classList.toggle('open');panelSelect.setAttribute('aria-expanded',String(open));});
document.addEventListener('click',e=>{if(panelDropdown && !panelDropdown.contains(e.target)){panelDropdown.classList.remove('open');panelSelect?.setAttribute('aria-expanded','false')}});
$$('.panel-option').forEach(option=>option.addEventListener('click',()=>{panelDropdown?.classList.remove('open');panelSelect?.setAttribute('aria-expanded','false')}));
...
function afterAuth(){
  ...
  // Get Started / Client Area / panel choices land in the separate dashboard.
  openClient('overview');
}
$$('.auth-required').forEach(el=>el.addEventListener('click',e=>{
  e.preventDefault();
  const action=el.dataset.authAction || 'client';
  if(getUser()){
    if(action==='checkout') openCheckout();
    else openClient('overview');
  }else{
    openAuth(action);
  }
}));
